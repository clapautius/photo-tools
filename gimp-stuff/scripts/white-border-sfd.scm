(define (script-fu-white-border-sfd image use-default use-default-small use-xtr
                                    black-width-x black-width-y
                                    white-width-x white-width-y
                                    shadow-width xtr-w-x xtr-w-y show-params)
  (let* ((img-w (car (gimp-image-width image))) ;; current image width
    ...
    (if (= use-default TRUE)
        (begin
          (set! black-width-x (floor (+ (/ img-max 1000) (if (< img-max 1000) 1 0))))
          (set! black-width-y (floor (+ (/ img-max 1000) (if (< img-max 1000) 1 0))))
          (if (= use-default-small TRUE)
              (begin
                (set! white-width-x (floor (/ img-max 100)))
                (set! white-width-y (floor (/ img-max 100)))
                (set! shadow-width (floor (/ (/ img-max 100) 1.3))))
              (begin                
    ...
    (let* (
           ...
           (img-supp-x (+ black-width-x white-width-x shadow-width xtr-w-x))
           (img-supp-y (+ black-width-y white-width-y shadow-width xtr-w-y))
      ...
      (gimp-image-add-layer image new-layer (car (gimp-image-get-layers image)))
...
(script-fu-register
 ...
 "White borders with shadow - SFD - v1.0"
 "Tudor M. Pristavu"
 ...
 SF-TOGGLE "Use default small params." TRUE
